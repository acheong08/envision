<script lang="ts">
	...
	async function submit() {
		...
		const response = await fetch('https://duti.tech/proxy/api/1/upload', {
			method: 'POST',
			body: formData
		});
...
<div class="container mx-8 my-10">
	<div class="grid grid-cols-3 gap-3">
		<div class="col-span-2">
			...
			<InputText placeholder="Link to source code" maxlength={2000} bind:value={model_source} />
			<h2 class="text-2xl">Download</h2>
			<InputText placeholder="Link to download" maxlength={2000} bind:value={model_download} />
		</div>
	...
	<button class="btn btn-primary btn-2xl w-full mt-5" on:click={submit}>Submit</button>
</div>
